...
impl CryptFS {
    ...
    fn crypt_read_file(&self, file: &fs::File) -> Vec<u8> {
        let file_size = file.metadata().unwrap().len();

        match self.mode {
            CryptMode::Encrypt => {
                let buf_size = self.get_crypt_read_size(file);
    ...
    /// # Panics
    /// Panics if the file cannot be read
    fn get_crypt_read_size(&self, file: &fs::File) -> u64 {
    ...
    fn crypt_translate(&self, data: &Vec<u8>, orig_size: u64) -> Result<Vec<u8>,()> {
